...
export default function DaftarGuru({ auth, gurus }) {
    const [daftarGuru, setDaftarGuru] = useState(gurus);
    const [searchInput, setSearchInput] = useState("");
    const [jumlahGuru, setJumlahGuru] = useState(0);

    const filterGurus = (guru) => {
        return guru.name.toLowerCase().includes(searchInput.toLowerCase());
    };
    const filteredGurus = searchInput ? gurus.filter(filterGurus) : gurus;

    const hapusGuru = (userId) => {
        if (window.confirm("Anda yakin ingin menghapus pengguna ini?")) {
            axios
                .delete(`/hapusguru/${userId}`)
                .then(() => {
                    // Perbarui daftar guru setelah penghapusan berhasil
                    setDaftarGuru(
                        daftarGuru.filter((guru) => guru.id !== userId)
                    );
